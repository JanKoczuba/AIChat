...
struct FirebaseAvatarService: RemoteAvatarService {
    ...
    func createAvatar(avatar: AvatarModel, image: UIImage) async throws {
        ...
        let url = try await FirebaseImageUploadService().uploadImage(
            image: image,
            path: path
        )
        ...
        // Upload the avatar
        try collection.document(avatar.avatarId).setData(
            from: avatar,
            merge: true
        )
    }
    ...
    func incrementAvatarClickCount(avatarId: String) async throws {
        try await collection.document(avatarId).updateData([
            AvatarModel.CodingKeys.clickCount.rawValue: FieldValue.increment(
                Int64(1)
            )
        ])
    }
